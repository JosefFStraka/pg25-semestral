...
ShaderProgram::ShaderProgram(const std::filesystem::path & VS_file, const std::filesystem::path & FS_file, bool hehe) :
...
void ShaderProgram::setUniform(const std::string& name, const glm::vec3& in_vec3) {
    auto loc = getUniformLocation(name);
    glProgramUniform3fv(ID, loc, 1, glm::value_ptr(in_vec3));
}
