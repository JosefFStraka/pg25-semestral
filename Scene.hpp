...
private:
    ...
    void load_static_aabbs(ModelInstance* model_inst_ptr, std::vector<AABB>& aabbs, AssetManager& assets) {
        ...
        if (model_res && !model_res->meshes.empty()) {
            ...
            if (all_loaded) {
                for (auto& mesh_pkg : model_res->meshes) {
                    ...
                    glm::mat4 local_transform = model_inst_ptr->createMM(mesh_pkg.origin, mesh_pkg.eulerAngles, mesh_pkg.scale);
                    glm::mat4 global_transform = local_transform * model_inst_ptr->local_model_matrix;
    ...
    bool check_dynamic_collision(ModelInstance* model_inst_ptr, const AABB& bounds, AssetManager& assets) {
        auto model_res = assets.getResourceMaybe(model_inst_ptr->model);
        if (!model_res) return false;
...
public:
    ...
    std::unordered_map<std::string, ModelInstance>& get_all_models() { return models; }
    std::unordered_map<std::string, ModelInstance*>& get_models() { return enabled_models; }

    void add_model(const std::string& name, ModelInstance model, bool enabled = true) {
    ...
    void add_static_model(const std::string& name, ModelInstance model, AssetManager& assets, bool enabled = true) {
        model.prepare();
        ...
        if (model_res) {
            for (auto& mesh_pkg : model_res->meshes) {
                auto mesh = assets.getResourceMaybe(mesh_pkg.mesh);
                if (mesh) {
                    glm::mat4 local_transform = model.createMM(mesh_pkg.origin, mesh_pkg.eulerAngles, mesh_pkg.scale);
                    glm::mat4 global_transform = local_transform * model.local_model_matrix;
                    aabbs.push_back(mesh->aabb_.transform(global_transform));
                }
            }
        }
    ...
    void update_shader_lights(ShaderProgram* shader) {
        if (shader) {
            ...
            shader->setUniform("lights.attenuation", MAX_LIGHTS, &lights.attenuation[0]);
            shader->setUniform("lights.spotCutoff", MAX_LIGHTS, &lights.spotCutoff[0]);
        }
    }
};
